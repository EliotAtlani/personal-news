...
class NewsScheduler:
    ...
    async def generate_daily_digest(self):
        ...
        try:
            ...
            filtered_articles = self.content_filter.filter_articles(articles, self.config.topics)
            ...
            if len(filtered_articles) < min_articles:
                logger.warning(f"Only {len(filtered_articles)} articles found, but minimum {min_articles} required")
                # Lower the relevance score threshold to get more articles
                self.content_filter.min_relevance_score = max(0.3, self.content_filter.min_relevance_score - 0.2)
                filtered_articles = self.content_filter.filter_articles(articles, self.config.topics)
                logger.info(f"Lowered relevance threshold, now have {len(filtered_articles)} articles")
            ...
            summaries = await self.summarizer.summarize_articles(
                filtered_articles,
                summary_length=self.config.content.summary_length
            )
    ...
    def _log_digest_stats(self, summaries, categories):
        ...
        for category, articles in categories.items():
            ...
            logger.info(f"  {category}: {len(articles)} articles (avg importance: {avg_importance:.2f})")
        
        if summaries:
            top_article = max(summaries, key=lambda s: s.importance_score)
            logger.info(f"Top article: '{top_article.article.title}' (score: {top_article.importance_score:.2f})")
    ...
    def schedule_test_run(self, delay_minutes: int = 1):
        ...
        logger.info(f"Test digest scheduled for {run_time.strftime('%Y-%m-%d %H:%M:%S')}")
