...
def setup_config():
    ...
    if config_path.exists():
        response = input(
            f"Configuration already exists at {config_path}. Overwrite? (y/N): "
        )
        if response.lower() != "y":
    ...
    print(
        "Examples: artificial intelligence, climate change, technology, space exploration"
    )
    ...
    # schedule
    schedule_time = (
        input("Daily send time (HH:MM, 24-hour format, default 08:00): ").strip()
        or "08:00"
    )
    ...
    config = {
        "user": {"email": user_email, "name": user_name, "timezone": "UTC"},
        "topics": topics,
        "sources": [
            "bbc-news",
            "reuters",
        ...
        "schedule": {"time": schedule_time, "enabled": True},
        "content": {
            "max_articles": 10,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Personal News - AI-powered daily digest"
    )
