...
class ContentFilter:
    ...
    def filter_articles(self, articles: List[Article], topics: List[str]) -> List[Article]:
        ...
        for current_article in articles:
            ...
            for existing_article in unique_articles:
                ...
                if self._are_similar_articles(current_article, existing_article):
                    ...
                    if current_article.relevance_score > existing_article.relevance_score:
    ...
    def _are_similar_articles(self, article1: Article, article2: Article) -> bool:
        ...
        if article1.description and article2.description:
            desc_similarity = SequenceMatcher(
                None, 
                article1.description.lower(), 
                article2.description.lower()
            ).ratio()
        ...
        if self._same_story_different_source(article1.url, article2.url):
            return True
    ...
    def _same_story_different_source(self, url1: str, url2: str) -> bool:
        ...
        try:
            ...
            if parsed1.netloc != parsed2.netloc:
                path1_parts = [part for part in parsed1.path.split('/') if len(part) > 3]
                path2_parts = [part for part in parsed2.path.split('/') if len(part) > 3]
...
class TopicMatcher:
    ...
    def _expand_topics(self, topics: List[str]) -> Dict[str, List[str]]:
        ...
        expansions = {
            'artificial intelligence': ['ai', 'machine learning', 'deep learning', 'neural network', 'chatgpt', 'gpt'],
            'climate change': ['global warming', 'carbon emission', 'renewable energy', 'sustainability', 'carbon footprint'],
            'technology': ['tech', 'software', 'hardware', 'innovation', 'digital', 'computing'],
            'space exploration': ['nasa', 'spacex', 'mars', 'rocket', 'satellite', 'astronaut', 'space station'],
            'renewable energy': ['solar', 'wind energy', 'hydroelectric', 'clean energy', 'green energy', 'sustainable']
        }
