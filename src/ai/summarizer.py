...
class ArticleSummary:
    def __init__(self, article: Article, brief_summary: str, key_points: List[str], 
                 category: str = "General", importance_score: float = 0.5):
...
class OpenAISummarizer:
    ...
    async def summarize_article(self, article: Article, summary_length: str = "medium") -> ArticleSummary:
        ...
        try:
            ...
            response = await asyncio.to_thread(
                ...
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a professional news summarizer. Provide concise, accurate summaries and analysis."},
            ...
            return self._parse_openai_response(article, response.choices[0].message.content)
            
        except Exception as e:
            logger.error(f"Error summarizing article with OpenAI: {e}")
            return self._fallback_summary(article)
    
    def _parse_openai_response(self, article: Article, response_text: str) -> ArticleSummary:
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith('SUMMARY:'):
                    ...
                    try:
                        importance_score = float(line.replace('IMPORTANCE:', '').strip())
                    except ValueError:
                        importance_score = 0.5
            ...
            if not key_points:
                key_points = [f"Source: {article.source}", f"Published: {article.published_at.strftime('%Y-%m-%d')}"]
            
            return ArticleSummary(article, summary, key_points, category, importance_score)
...
class AnthropicSummarizer:
    ...
    def _parse_anthropic_response(self, article: Article, response_text: str) -> ArticleSummary:
...
class GeminiSummarizer:
    ...
    def _parse_gemini_response(self, article: Article, response_text: str) -> ArticleSummary:
            ...
            for line in lines:
                ...
                if line.startswith('SUMMARY:'):
                    ...
                    if current_section == 'key_points' or not current_section:
                        key_points.append(line.lstrip('•-* ').strip())
                elif current_section == 'key_points' and line and not line.startswith(('CATEGORY:', 'IMPORTANCE:')):
                    key_points.append(line)
    ...
    def _simple_categorize(self, article: Article) -> str:
        ...
        categories = {
            ...
            'Science': ['research', 'study', 'scientist', 'discovery', 'climate', 'space'],
            'Business': ['company', 'business', 'market', 'economy', 'financial', 'stock'],
            'Health': ['health', 'medical', 'hospital', 'disease', 'treatment', 'vaccine'],
            'Politics': ['government', 'political', 'election', 'president', 'congress', 'policy'],
...
class NewsSummarizer:
    def __init__(self, openai_key: str = None, anthropic_key: str = None, gemini_key: str = None):
        self.openai_summarizer = OpenAISummarizer(openai_key) if openai_key else None
        self.anthropic_summarizer = AnthropicSummarizer(anthropic_key) if anthropic_key else None
        self.gemini_summarizer = GeminiSummarizer(gemini_key) if gemini_key else None
        
        if not self.openai_summarizer and not self.anthropic_summarizer and not self.gemini_summarizer:
            raise ValueError("At least one AI service (OpenAI, Anthropic, or Gemini) API key must be provided")
    
    async def summarize_articles(self, articles: List[Article], summary_length: str = "medium", 
                               max_concurrent: int = 5) -> List[ArticleSummary]:
        ...
        async def summarize_single(article: Article) -> ArticleSummary:
            async with semaphore:
                ...
                if self.gemini_summarizer:
                    try:
                        return await self.gemini_summarizer.summarize_article(article, summary_length)
                    except Exception as e:
                        logger.warning(f"Gemini summarization failed, trying OpenAI: {e}")
                
                # Fallback to OpenAI
                if self.openai_summarizer:
                    try:
                        return await self.openai_summarizer.summarize_article(article, summary_length)
                    except Exception as e:
                        logger.warning(f"OpenAI summarization failed, trying Anthropic: {e}")
                
                # Fallback to Anthropic
                if self.anthropic_summarizer:
                    try:
                        return await self.anthropic_summarizer.summarize_article(article, summary_length)
                    except Exception as e:
    ...
    def group_summaries_by_category(self, summaries: List[ArticleSummary]) -> Dict[str, List[ArticleSummary]]:
