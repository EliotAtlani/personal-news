...
class S3PreferencesManager:
    def __init__(
        self, bucket_name: str = None, preferences_key: str = "preferences.json"
    ):
        """
        ...
        if not self.bucket_name:
            raise ValueError(
                "S3 bucket name must be provided either as parameter or S3_BUCKET_NAME env var"
            )
    ...
    def load_preferences(self) -> dict[str, Any]:
        ...
        try:
            response = self.s3_client.get_object(
                Bucket=self.bucket_name, Key=self.preferences_key
            )
    ...
    def _get_default_preferences(self) -> dict[str, Any]:
        ...
        """
        return {
            "user": {"email": "", "name": "User", "timezone": "UTC"},
            "topics": [],
            "sources": [],
            "schedule": {"time": "08:00", "enabled": True},
            "content": {
                "max_articles": 10,
                "min_articles": 1,
